# -*- coding: utf-8 -*-
"""
This code reproduces Figures 4a-c and Supplementary Figures 23-30 from the publication:
...
if saveFigs == True:
    plt.savefig(os.path.join(path_figures,'phosphostates_Copeland_vs_model.svg'), bbox_inches = "tight")

#%% control experiment with fixed phosphatase ratios (Supplementary Figure 23)
...
if saveFigs == True:
    plt.savefig(os.path.join(path_figures,'phosphostates_Copeland_vs_model_ctrl.svg'), bbox_inches = "tight")
...
for i in range(5):
    ...
    except:  print('Shapiro test failed!')
...
# individual pts
for i in range(5):
    y = enzymesDonorHearts[:,i]
    y[y == 0] = 1e-11
    y = np.log10(y) - min10 + bullseye
    for ii in range(y.shape[0]):
        ax.plot(thetaCoords[idcs1[i]],y[ii], 'o', color = colorV[0],alpha=0.5)
    y = enzymesFailingHearts[:,i]
    y[y == 0] = 1e-11
    y = np.log10(y) - min10 + bullseye
    for ii in range(y.shape[0]):
        ax.plot(thetaCoords[idcs2[i]],y[ii], 'o', color = colorV[1],alpha=0.5)
...
if saveFigs == True:
    plt.savefig(os.path.join(path_figures,'EnzymesFitted_Copeland_vs_model.svg'), bbox_inches = "tight")
...
kin_donor = enzymesDonorHearts[:,0] 
kin_HF = enzymesFailingHearts[:,0] 
...
plt.plot(np.zeros(len(totPkaPPratio_donor)),totPkaPPratio_donor,'o',color=colorV[0],alpha=a)
plt.plot(np.ones(len(totPkaPPratio_HF)),totPkaPPratio_HF,'o',color=colorV[1],alpha=a)
...
if saveFigs == True:
    plt.savefig(os.path.join(path_figures,'totalKinPPaseRatioFitted_Copeland_vs_model.svg'), bbox_inches = "tight")
...
if saveFigs == True:
    plt.savefig(os.path.join(path_figures,'maxVals.svg'), bbox_inches = "tight")
...
#%% Polar coordinate plots of enzyme vectors after optimization (Supplementary Figures 24-28)
...
for d in range(len(datasets)):
    ...
    if saveFigs == True:
        plt.savefig(os.path.join(path_figures,ttlsDat[d]+'_enzymeVectors.svg'), bbox_inches = "tight")
...
for c in [2e-7,2e-6,2e-5]:

    # Load fitted/predicted values concentrations for matching Copeland et al 2010 heart data
    fitvalsDonorHearts = np.load(os.path.join(path_results,'fittedMyBPCstates_Copeland_DonorHearts_tQSSA_'+str(np.format_float_scientific(c, unique=False, precision=2))+'.npy'))
    fitvalsFailingHearts = np.load(os.path.join(path_results,'fittedMyBPCstates_Copeland_FailingHearts_tQSSA_'+str(np.format_float_scientific(c, unique=False, precision=2))+'.npy'))
    predvalsFailingHearts = np.load(os.path.join(path_results,'predictedMyBPCstates_Copeland_FailingHearts_tQSSA_'+str(np.format_float_scientific(c, unique=False, precision=2))+'.npy'))
    
    fitvalsDonorHeartsCtrl = np.load(os.path.join(path_results,'fittedMyBPCstates_Copeland_DonorHearts_ctrl_tQSSA_'+str(np.format_float_scientific(c, unique=False, precision=2))+'.npy'))
    fitvalsFailingHeartsCtrl = np.load(os.path.join(path_results,'fittedMyBPCstates_Copeland_FailingHearts_ctrl_tQSSA_'+str(np.format_float_scientific(c, unique=False, precision=2))+'.npy'))
    ...
    ax.set_xlim(-0.25,2.25)
    plt.legend(bbox_to_anchor=(1,1), loc="upper left") 
    plt.title('tQSSA model, [cMyBPC$_{total}$] = ' + str(np.format_float_scientific(c, unique=False, precision=2))+ ' mol/L')
    
    if saveFigs == True:
        plt.savefig(os.path.join(path_figures,'phosphostates_Copeland_vs_model_tQSSA_' + str(np.format_float_scientific(c, unique=False, precision=2))+'.svg'), bbox_inches = "tight")
    ...
    enzymesDonorHearts = np.load(os.path.join(path_results,'fittedEnzymeConcentrations_Copeland_DonorHearts_tQSSA_'+str(np.format_float_scientific(c, unique=False, precision=2))+'.npy'))
    enzymesDonorHearts = enzymesDonorHearts[:,2:] # PKA, PKC, PP1, total Phosphatases, RSK2 
    enzymesDonorHearts[:,3] = enzymesDonorHearts[:,3] - enzymesDonorHearts[:,2] # PKA, PKC, PP1, PP2A, RSK2 
    enzymesDonorHearts[enzymesDonorHearts < 1e-15] = 0 

    enzymesFailingHearts = np.load(os.path.join(path_results,'fittedEnzymeConcentrations_Copeland_FailingHearts_tQSSA_'+str(np.format_float_scientific(c, unique=False, precision=2))+'.npy'))
    enzymesFailingHearts = enzymesFailingHearts[:,2:] # PKA, PKC, PP1, total Phosphatases, RSK2 
    enzymesFailingHearts[:,3] = enzymesFailingHearts[:,3] - enzymesFailingHearts[:,2] # PKA, PKC, PP1, PP2A, RSK2 
    enzymesFailingHearts[enzymesFailingHearts < 1e-15] = 0 
    ...
    print('######################################################################################')       
    print('Statistical analysis of enzyme vectors healthy/HF donors,tQSSA, [cMyBPC] = '+str(np.format_float_scientific(c, unique=False, precision=2)))
    print('###################################################################################### \n \n')  
    ...
    if saveFigs == True:
        plt.savefig(os.path.join(path_figures,'EnzymesFitted_Copeland_vs_model_tQSSA_' + str(np.format_float_scientific(c, unique=False, precision=2))+'.svg'), bbox_inches = "tight")
        
    # Kinase/Phosphatase ratio (inset)
    ...
    if saveFigs == True:
        plt.savefig(os.path.join(path_figures,'totalKinPPaseRatioFitted_Copeland_vs_model_tQSSA_' + str(np.format_float_scientific(c, unique=False, precision=2))+'.svg'), bbox_inches = "tight")



#%% Max possible fractions (Supplementary Figure S30)
...
for c in [2e-7,2e-6,2e-5]:
    ...
    optResult_healthy = np.load(os.path.join(path_results,'optResult_healthy_tQSSA_'+str(np.format_float_scientific(c, unique=False, precision=2))+'.npy'))
    optResult_HF = np.load(os.path.join(path_results,'optResult_HF_tQSSA_'+str(np.format_float_scientific(c, unique=False, precision=2))+'.npy'))
    optResult_HF_fixPP1 = np.load(os.path.join(path_results,'optResult_HF_PP1fixed_tQSSA_'+str(np.format_float_scientific(c, unique=False, precision=2))+'.npy'))
    optResult_HF_fixPP2A = np.load(os.path.join(path_results,'optResult_HF_PP2Afixed_tQSSA_'+str(np.format_float_scientific(c, unique=False, precision=2))+'.npy'))
    optResult_HF_PPfree = np.load(os.path.join(path_results,'optResult_HF_PPasesFree_tQSSA_'+str(np.format_float_scientific(c, unique=False, precision=2))+'.npy'))
    ...
    plt.yticks(fontsize=14)
    plt.ylabel('Maximum possible fraction',fontsize=16)
    plt.title('[cMyBPC$_{total}$] = '+str(np.format_float_scientific(c, unique=False, precision=2))+' mol/L')
    plt.ylim([0,1.33])
    plt.yticks([0,0.2,0.4,0.6,0.8,1])
    path = os.path.join(fileDirectory,'_')
    ...
    if saveFigs == True:
        plt.savefig(os.path.join(path_figures,'maxVals_tQSSA_'+str(np.format_float_scientific(c, unique=False, precision=2))+'.svg'), bbox_inches = "tight")
